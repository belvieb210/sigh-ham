/**
 * Crée ou met à jour le compte laborantin
 * Usage : npx tsx prisma/seed-utilisateur-laboratoire.ts
 */
...
async function main() {
  ...
  if (role.salle?.code !== "LABORATOIRE") {
    throw new Error(
      `Le rôle LABORANTIN n'est pas lié à la salle LABORATOIRE (salle actuelle: ${role.salle?.code ?? "aucune"}).`
    );
  }
