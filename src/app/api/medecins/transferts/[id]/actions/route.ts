...
import {
  confirmerTransfertMedecins,
  rejeterTransfertMedecins,
  restaurerTransfertMedecins,
} from "@/lib/medecins/gestion-transfert-medecins";
...
export async function POST(
  ...
  try {
    ...
    switch (action) {
      case "confirmer":
        resultat = await confirmerTransfertMedecins(
          session.utilisateur.id,
          transfertId
        );
        break;
      case "rejeter":
        resultat = await rejeterTransfertMedecins(
          session.utilisateur.id,
          transfertId,
          body.motifRejet
        );
        break;
      case "restaurer":
        resultat = await restaurerTransfertMedecins(
          session.utilisateur.id,
          transfertId
        );
        break;
    ...
    console.error("[POST /api/medecins/transferts/[id]/actions]", error);
    let message =
