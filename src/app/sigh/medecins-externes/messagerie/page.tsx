import type { Metadata } from "next";
import { ContenuMessagerieMedecinsExternes } from "@/features/medecins-externes/contenu-messagerie-medecins-externes";
import { verifierAccesMedecinsExternes } from "@/lib/auth/garde-salle";
import { propsUtilisateurMedecinsExternes } from "@/lib/auth/props-utilisateur-medecins-externes";

export const metadata: Metadata = {
  title: "Messagerie — Médecins externes",
  robots: { index: false, follow: false },
};

export default async function Page() {
  const utilisateur = await verifierAccesMedecinsExternes();
  const props = propsUtilisateurMedecinsExternes(utilisateur);
  return (
    <ContenuMessagerieMedecinsExternes
      utilisateur={{ ...props, id: utilisateur.id }}
      estAdmin={utilisateur.role.code === "ADMIN" || utilisateur.role.code === "SUPER_ADMIN"}
    />
  );
}
