...
import {
  compterFiltresMensuel,
  filtresMensuelVides,
  FormulaireFiltresRapportMensuelCaisse,
  type FiltresRapportMensuelUi,
} from "@/features/caisse/formulaire-filtres-rapport-mensuel-caisse";
...
export function ContenuRapportMensuelCaisse({ utilisateur }: Props) {
  const { t } = useTranslation();
  const moisDefaut = useMemo(() => moisCourantIso(), []);
  const [rapport, setRapport] = useState<RapportCaissePayload | null>(null);
  const [chargement, setChargement] = useState(true);
  const [erreur, setErreur] = useState<string | null>(null);
  const [filtresOuverts, setFiltresOuverts] = useState(false);
  const [brouillon, setBrouillon] = useState<FiltresRapportMensuelUi>(() =>
    filtresMensuelVides(moisDefaut)
  );
  const [appliques, setAppliques] = useState<FiltresRapportMensuelUi>(() =>
    filtresMensuelVides(moisDefaut)
  );
  const [page, setPage] = useState(1);

  const charger = useCallback(async (f: FiltresRapportMensuelUi) => {
    setChargement(true);
    setErreur(null);
    try {
      const params = new URLSearchParams({ periode: "mensuel" });
      if (f.mois) params.set("mois", f.mois);
  ...
  const nbFiltres = compterFiltresMensuel(appliques, moisDefaut);
  ...
  return (
    <MiseEnPageCaisse
      ...
      titre={t("caisse.rapports.mensuel.titre")}
      sousTitre={t("caisse.rapports.mensuel.sousTitre")}
    >
      <div className="mx-auto w-full max-w-7xl space-y-4 pb-8">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold text-texte-principal">
              {t("caisse.rapports.mensuel.titre")}
            </h2>
            <p className="mt-1 text-sm capitalize text-texte-secondaire">
              {rapport?.labelPeriode ?? t("caisse.rapports.mensuel.sousTitre")}
            </p>
          </div>
        ...
        {filtresOuverts && (
          <FormulaireFiltresRapportMensuelCaisse
            ...
            onReinitialiser={() => {
              const vides = filtresMensuelVides(moisDefaut);
              setBrouillon(vides);
              setAppliques(vides);
            }}
          />
