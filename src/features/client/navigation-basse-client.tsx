...
export function NavigationBasseClient({ onMenu }: { onMenu: () => void }) {
  const { t } = useTranslation();
  const pathname = usePathname();
  const { basse } = useNavigationClient();
  ...
  return (
    <nav
      ...
      aria-label={t("client.layout.navigationMobile")}
    >
      <ul className="flex items-stretch justify-around px-1 pt-1">
        {basse.map((item) => {
          const actif =
            pathname === item.href ||
            (item.id === "accueil" && pathname === "/sigh/client") ||
        ...
        <li className="flex-1">
          ...
          >
            ...
            <span>{t("client.navBas.menu")}</span>
          </button>
        </li>
      </ul>
    </nav>
