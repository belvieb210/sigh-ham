...
import { EVENEMENT_EGLISE_PATIENTS_MODIFIES } from "@/constants/eglise";
...
function PanneauEstimationConvention() {
  ...
  const chargerEstimation = useCallback(async (dossierId: string) => {
    try {
      const res = await fetch(
        `/api/eglise/estimations?dossierId=${encodeURIComponent(dossierId)}`
      );
  ...
  const envoyerCaisse = async (estimationId: string) => {
    ...
    setMessage(null);
    try {
      const res = await fetch(`/api/eglise/estimations/${estimationId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      window.dispatchEvent(new CustomEvent(EVENEMENT_EGLISE_PATIENTS_MODIFIES));
      if (dossierIds.length === 1) {
        const est = await chargerEstimation(dossierIds[0]!);
        setEstimation(est);
      }
  ...
  const uploadPdf = async (file: File, estimationId: string) => {
    ...
    try {
      const res = await fetch("/api/eglise/estimations/upload", {
        method: "POST",
        body: form,
      });
      const data = (await res.json()) as { message?: string };
      ...
      <section className="rounded-xl border border-gris-bordure bg-white p-4 shadow-sm">
        ...
        {dossierIds.length === 0 ? (
          ...
          <div className="space-y-2 text-sm">
            <p>{t("eglise.panneau.selectionMultiple", { count: dossierIds.length })}</p>
            <p>
          ...
          <div className="space-y-2 text-sm">
            ...
            <p className="text-xs text-texte-secondaire">Statut : {estimation.statut.replace("_", " ")}</p>
          </div>
        ) : (
          <Bouton taille="petit" onClick={() => void genererEstimation()} disabled={enCours}>
            <FileText className="mr-1 h-4 w-4" />
            {t("eglise.panneau.genererEstimation")}
          </Bouton>
        )}
          ...
          <div className="flex flex-col gap-2">
            {estimationsActives.map((est) => (
              <div key={est.id} className="space-y-2 border-b border-gris-bordure pb-2 last:border-0">
                ...
                <a
                  href={`/api/eglise/estimations/${est.id}`}
                  target="_blank"
                  rel="noreferrer"
      ...
      {peutConfirmer && dossierIds.length === 1 && (
        <section className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4 shadow-sm">
          ...
          <p className="mb-3 text-xs text-emerald-900">
            Validez manuellement l&apos;envoi du patient vers la destination choisie.
          </p>
          <Bouton
            taille="petit"
...
export function PanneauDroitConvention() {
  const { t } = useTranslation();
  ...
  return (
    <div className="space-y-4">
      ...
      <PanneauEstimationConvention />
    </div>
  );
}

/** Sections panneau conventionné — mobile / tablette */
export function SectionsMobileConvention() {
  const { t } = useTranslation();
  const { resume } = useResumePatient();
