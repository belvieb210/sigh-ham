...
export interface ResumePatientInfirmiers {
  initiales: string;
  nomComplet: string;
...
export const RESUME_INFIRMIERS_VIDE: ResumePatientInfirmiers = {
  initiales: "—",
...
interface ContexteSelectionInfirmiers {
  patientSelectionne: PatientFileInfirmiers | null;
  resume: ResumePatientInfirmiers;
  selectionnerPatient: (patient: PatientFileInfirmiers) => void;
  ...
  synchroniserSelection: (patients: PatientFileInfirmiers[]) => void;
  modificationEnCours: boolean;
  messagePanneau: string | null;
}

const Contexte = createContext<ContexteSelectionInfirmiers | null>(null);

export function FournisseurSelectionInfirmiers({ children }: { children: ReactNode }) {
  const { definirOrientation, definirOrientations } = useOrientationInfirmiers();
  const [patientSelectionne, setPatientSelectionne] =
    useState<PatientFileInfirmiers | null>(null);
  const [resume, setResume] = useState<ResumePatientInfirmiers>(RESUME_INFIRMIERS_VIDE);
    ...
    async (codesSalle: string[]) => {
      ...
      try {
        const { ok, echecs, resultats, premierEchec } = await orienterPatientsEnSerie(
          cibles,
          async (dossierId) => {
            const res = await fetch("/api/infirmiers/transferts", {
              method: "POST",
        ...
        const label =
          ...
          codesFinal
            .map(
              (c) =>
                ORIENTATIONS_RAPIDES_INFIRMIERS.find((o) => o.value === c)?.labelKey ?? c
            )
            .join(", ");
        ...
        if (
          ...
          patientSelectionne.codeSalleDestination !== "INFIRMIERS"
        ) {
  ...
  useEffect(() => {
    debounceOrientationRef.current = creerDebounce((codes: string[]) => {
      void appliquerOrientationsInfirmiers(codes);
    }, 400);
    return () => debounceOrientationRef.current?.annuler();
  }, [appliquerOrientationsInfirmiers]);
  ...
  const synchroniserSelection = useCallback((patients: PatientFileInfirmiers[]) => {
    setPatientSelectionne((courant) => {
...
export function useSelectionInfirmiers() {
  const contexte = useContext(Contexte);
  if (!contexte) {
    throw new Error(
      "useSelectionInfirmiers doit être dans FournisseurSelectionInfirmiers."
    );
  }
  return contexte;
}

export function useSelectionInfirmiersOptionnel() {
