...
interface PropsBarreLateraleInfirmiers {
...
function LiensNavigation({
  ...
  const { tableauDeBord, infirmiers, communication, parametres } =
    useNavigationInfirmiers();

  const estActif = (href: string) => {
    if (href === "/sigh/infirmiers") return pathname === href;
    ...
    <>
      <div className="flex items-start justify-between border-b border-gris-bordure px-4 py-4">
        <div className="min-w-0 flex-1">
          <LogoHam taille="petit" href="/sigh/infirmiers" className="max-w-full" />
        ...
        {onFermer && (
          <button
            ...
            aria-label={t("infirmiers.layout.fermerMenu")}
          >
            <X className="h-5 w-5" />
          </button>
        )}
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        {rendreSection(
          t("infirmiers.layout.sectionTableauDeBord"),
          tableauDeBord as ItemNav[]
        )}
        {rendreSection(t("infirmiers.layout.sectionInfirmiers"), infirmiers as ItemNav[])}
        {rendreSection(
          t("infirmiers.layout.sectionCommunication"),
          communication as ItemNav[]
        )}
        {rendreSection(t("infirmiers.layout.sectionParametres"), parametres as ItemNav[])}
      </nav>

      <div className="space-y-2 border-t border-gris-bordure p-3">
        <Link
          href="/sigh/infirmiers/profil"
          onClick={onFermer}
        ...
        {onFermer && (
          ...
          >
            ...
            {t("infirmiers.layout.deconnecter")}
          </button>
        )}
      </div>
    </>
...
export function BarreLateraleInfirmiers({
  utilisateur,
  ouvert = false,
  onFermer,
  badgeFile = 0,
}: PropsBarreLateraleInfirmiers) {
  const { t } = useTranslation();
...
interface PropsEnTeteInfirmiers {
  titre: string;
  sousTitre: string;
  utilisateur: UtilisateurInfirmiers;
  onMenu?: () => void;
...
export function EnTeteInfirmiers({
  titre,
  sousTitre,
  utilisateur,
  onMenu,
}: PropsEnTeteInfirmiers) {
  const { t } = useTranslation();
    ...
    <header className="sticky top-0 z-30 shrink-0 border-b border-gris-bordure bg-white">
      <div className="flex items-center gap-1.5 px-2.5 py-2 sm:gap-2 sm:px-4 sm:py-2.5 lg:hidden">
        <button
          ...
          aria-label={t("infirmiers.layout.ouvrirMenu")}
        >
          <Menu className="h-5 w-5" />
        </button>
        <LogoHam
          taille="petit"
          href="/sigh/infirmiers"
          afficherTexte={false}
        ...
        <MenuProfilEntete utilisateur={utilisateur} compact hrefProfil="/sigh/infirmiers/profil" />
      </div>
      ...
      <div className="hidden items-center justify-between gap-4 px-6 py-4 lg:flex">
        ...
        <div className="relative hidden min-w-0 flex-1 max-w-md xl:block">
          ...
          <input
            ...
            placeholder={t("infirmiers.layout.rechercher")}
          ...
          <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 rounded border border-gris-bordure bg-white px-1.5 py-0.5 text-[10px] text-texte-secondaire">
            {t("infirmiers.layout.raccourciRecherche")}
          </span>
        </div>
        ...
        <div className="flex items-center justify-end gap-3">
          <SelecteurLangue />
          <BoutonNotificationsEnTete />
          <MenuProfilEntete utilisateur={utilisateur} hrefProfil="/sigh/infirmiers/profil" />
        </div>
      </div>
    </header>
  );
