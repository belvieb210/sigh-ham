...
interface PropsMenuActionsTransfertInfirmiers {
  patient: PatientFileInfirmiers;
  onRafraichir?: () => void;
}

export function MenuActionsTransfertInfirmiers({
  patient,
  onRafraichir,
}: PropsMenuActionsTransfertInfirmiers) {
  ...
  const executerAction = async (action: "confirmer" | "rejeter" | "restaurer") => {
    ...
    try {
      const res = await fetch(
        `/api/infirmiers/transferts/${encodeURIComponent(patient.transfertSortantId)}/actions`,
        {
          method: "POST",
      ...
      if (!res.ok) throw new Error(data.message ?? t("infirmiers.actions.actionImpossible"));
      ...
      setErreur(
        error instanceof Error ? error.message : t("infirmiers.actions.erreurInattendue")
      );
    } finally {
      setEnCours(false);
    }
  ...
  return (
    <>
      <button
        ...
        aria-label={t("infirmiers.actions.plusActions", { nom: patient.nomComplet })}
          ...
          >
            {peutConfirmer && (
              ...
              >
                ...
                <span>{t("infirmiers.actions.confirmer")}</span>
              </button>
            )}
            {peutRejeter && (
              <button
              ...
              >
                ...
                <span>{t("infirmiers.actions.rejeter")}</span>
              </button>
            )}
            {peutRestaurer && (
              <button
