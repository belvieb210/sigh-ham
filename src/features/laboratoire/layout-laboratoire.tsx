...
interface PropsBarreLateraleLaboratoire {
...
function LiensNavigation({
  ...
  const { tableauDeBord, laboratoire, rapports, communication, parametres } =
    useNavigationLaboratoire();

  const estActif = (href: string) => {
    if (href === "/sigh/laboratoire") return pathname === href;
    ...
    <>
      <div className="flex items-start justify-between border-b border-gris-bordure px-4 py-4">
        <div className="min-w-0 flex-1">
          <LogoHam taille="petit" href="/sigh/laboratoire" className="max-w-full" />
        ...
        {onFermer && (
          <button
            ...
            aria-label={t("laboratoire.layout.fermerMenu")}
          >
            <X className="h-5 w-5" />
          </button>
        )}
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        {rendreSection(
          t("laboratoire.layout.sectionTableauDeBord"),
          tableauDeBord as ItemNav[]
        )}
        {rendreSection(
          t("laboratoire.layout.sectionLaboratoire"),
          laboratoire as ItemNav[]
        )}
        {rendreSection(
          t("laboratoire.layout.sectionRapports"),
          rapports as ItemNav[]
        )}
        {rendreSection(
          t("laboratoire.layout.sectionCommunication"),
          communication as ItemNav[]
        )}
        {rendreSection(
          t("laboratoire.layout.sectionParametres"),
          parametres as ItemNav[]
        )}
      </nav>

      <div className="space-y-2 border-t border-gris-bordure p-3">
        <Link
          href="/sigh/laboratoire/profil"
          onClick={onFermer}
        ...
        {onFermer && (
          ...
          >
            ...
            {t("laboratoire.layout.deconnecter")}
          </button>
        )}
      </div>
    </>
...
export function BarreLateraleLaboratoire({
  utilisateur,
  ouvert = false,
  onFermer,
  badgeFile = 0,
...
interface PropsEnTeteLaboratoire {
  titre: string;
  sousTitre: string;
  utilisateur: UtilisateurLaboratoire;
  onMenu?: () => void;
    ...
    <header className="sticky top-0 z-30 shrink-0 border-b border-gris-bordure bg-white">
      <div className="flex items-center gap-1.5 px-2.5 py-2 sm:gap-2 sm:px-4 sm:py-2.5 lg:hidden">
        <button
          ...
          aria-label={t("laboratoire.layout.ouvrirMenu")}
        >
          <Menu className="h-5 w-5" />
        </button>
        <LogoHam
          taille="petit"
          href="/sigh/laboratoire"
          afficherTexte={false}
        ...
        <MenuProfilEntete
          utilisateur={utilisateur}
          compact
          hrefProfil="/sigh/laboratoire/profil"
        />
      </div>
