...
interface PropsMenuActionsTransfertLaboratoire {
  patient: PatientFileLaboratoire;
  onRafraichir?: () => void;
}

/**
 * Menu ⋮ comme à la caisse / réception : Confirmer / Rejeter / Restaurer.
 * Visible dès qu'un transfert sortant existe (orientation rapide).
 */
export function MenuActionsTransfertLaboratoire({
  patient,
  onRafraichir,
}: PropsMenuActionsTransfertLaboratoire) {
  ...
  const executerAction = async (action: "confirmer" | "rejeter" | "recuperer") => {
    ...
    try {
      const res = await fetch(
        `/api/laboratoire/transferts/${encodeURIComponent(patient.transfertSortantId)}/actions`,
        {
          method: "POST",
      ...
      if (!res.ok) {
        throw new Error(data.message ?? t("laboratoire.transferts.actionImpossible"));
      }
      ...
      setErreur(
        error instanceof Error
          ? error.message
          : t("laboratoire.transferts.erreurInattendue")
      );
    } finally {
      setEnCours(false);
    }
  ...
  return (
    <>
      <button
        ...
        aria-label={t("laboratoire.transferts.plusActions", {
          ...
          >
            {peutConfirmer && (
              ...
              >
                ...
                <span>{t("laboratoire.transferts.confirmer")}</span>
              </button>
            )}
            {peutRejeter && (
              <button
              ...
              >
                ...
                <span>{t("laboratoire.transferts.rejeter")}</span>
              </button>
            )}
            {peutRestaurer && (
              <button
