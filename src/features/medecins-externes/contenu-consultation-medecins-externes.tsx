...
interface PropsContenuConsultationMedecinsExternes {
  utilisateur: UtilisateurMedecinsExternes;
}

export function ContenuConsultationMedecinsExternes({
  utilisateur,
}: PropsContenuConsultationMedecinsExternes) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (annule) return;
        if (!res.ok) {
          setErreur(data.erreur ?? t("medecinsExternes.erreurs.chargementPatients"));
          return;
        }
        setPatients(data.patients ?? []);
      } catch {
        if (!annule) setErreur(t("medecinsExternes.erreurs.chargementPatients"));
      } finally {
    ...
    (async () => {
      ...
      try {
        const [resConsult, resDetail] = await Promise.all([
          fetch(`/api/medecins-externes/consultations?dossierId=${encodeURIComponent(dossierId)}`),
          fetch(`/api/medecins-externes/patients/${encodeURIComponent(dossierId)}`),
        ]);
        ...
        if (!annule) setErreur(t("medecinsExternes.consultation.erreurChargement"));
      }
    })();
  ...
  async function sauvegarder() {
    if (!dossierId || !motif.trim()) {
      setErreur(t("medecinsExternes.consultation.motifRequis"));
      return;
    }
    setEnCours(true);
    setErreur(null);
    ...
    try {
      if (!consultation) {
        const res = await fetch("/api/medecins-externes/consultations", {
          method: "POST",
        ...
        setMessage(t("medecinsExternes.consultation.enregistree"));
      } else {
        const res = await fetch(`/api/medecins-externes/consultations/${consultation.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  async function cloturer() {
    ...
    setErreur(null);
    try {
      await fetch(`/api/medecins-externes/consultations/${consultation.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ motif, anamnese, examenClinique, conclusion }),
      });
      ...
      appliquerConsultation(data.consultation);
      setMessage(t("medecinsExternes.consultation.cloturee"));
    } catch {
      setErreur(t("medecinsExternes.actions.erreurInattendue"));
    } finally {
      setEnCours(false);
  ...
  async function ajouterDiag() {
    ...
    try {
      const res = await fetch(
        `/api/medecins-externes/consultations/${consultation.id}/diagnostics`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function supprimerDiag(id: string) {
    ...
    const res = await fetch(`/api/medecins-externes/diagnostics/${id}`, { method: "DELETE" });
    if (!res.ok) return;
    setConsultation({
      ...consultation,
  ...
  async function ajouterActeUi() {
    ...
    try {
      const res = await fetch(
        `/api/medecins-externes/consultations/${consultation.id}/actes`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function supprimerActeUi(id: string) {
    if (!consultation) return;
    const res = await fetch(`/api/medecins-externes/actes/${id}`, { method: "DELETE" });
    if (!res.ok) return;
    setConsultation({
      ...consultation,
  ...
  return (
    <MiseEnPageMedecinsExternes
      ...
      titre={t("medecinsExternes.consultation.titre")}
      sousTitre={t("medecinsExternes.consultation.sousTitre")}
    >
      <div className="mx-auto w-full max-w-[1100px] space-y-5">
        {chargement ? (
          <div className="flex items-center gap-2 text-sm text-texte-secondaire">
            <Loader2 className="h-4 w-4 animate-spin" />
            {t("medecinsExternes.consultation.chargement")}
          </div>
        ) : (
          <>
            <div className="rounded-xl border border-gris-bordure bg-white p-4 shadow-sm">
              <label className="text-xs font-medium text-texte-secondaire">
                {t("medecinsExternes.consultation.selectionPatient")}
              </label>
              <select
              ...
              <>
                <div className="grid gap-4 lg:grid-cols-3">
                  <div className="rounded-xl border border-gris-bordure bg-white p-4 shadow-sm lg:col-span-2">
                    ...
                    <p className="mt-1 text-xs text-texte-secondaire">
                      {t("medecinsExternes.consultation.dossier")}{" "}
                    ...
                    {constantes ? (
                      <p className="mt-3 text-xs text-texte-secondaire">
                        {t("medecinsExternes.consultation.constantes")} : T°{" "}
                      ...
                      <p className="mt-3 text-xs text-texte-secondaire">
                        {t("medecinsExternes.consultation.pasConstantes")}
                      </p>
                    )}
                  </div>
                  <div className="rounded-xl border border-gris-bordure bg-white p-4 shadow-sm">
                    <p className="text-xs text-texte-secondaire">
                      {cloturee
                        ? t("medecinsExternes.consultation.statutCloturee")
                        : consultation
                          ? t("medecinsExternes.consultation.statutOuverte")
                          : t("medecinsExternes.consultation.statutNouvelle")}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-3 rounded-xl border border-gris-bordure bg-white p-4 shadow-sm md:col-span-2">
                    <div>
                      <label className="text-xs font-medium text-texte-secondaire">
                        {t("medecinsExternes.consultation.motif")}
                      </label>
                      <input
                        className={`${champ} mt-1`}
                        value={motif}
                    ...
                    <div>
                      <label className="text-xs font-medium text-texte-secondaire">
                        {t("medecinsExternes.consultation.anamnese")}
                      </label>
                      <textarea
                    ...
                    <div>
                      <label className="text-xs font-medium text-texte-secondaire">
                        {t("medecinsExternes.consultation.examenClinique")}
                      </label>
                      <textarea
                        className={`${champ} mt-1 min-h-[80px]`}
                    ...
                    <div>
                      <label className="text-xs font-medium text-texte-secondaire">
                        {t("medecinsExternes.consultation.conclusion")}
                      </label>
                      <textarea
                        className={`${champ} mt-1 min-h-[80px]`}
                        value={conclusion}
                  ...
                  <div className="rounded-xl border border-gris-bordure bg-white p-4 shadow-sm">
                    <h4 className="text-sm font-semibold">
                      {t("medecinsExternes.consultation.diagnostics")}
                    </h4>
                    ...
                    {!cloturee && consultation && (
                      <div className="mt-3 space-y-2">
                        <input
                          className={champ}
                          placeholder={t("medecinsExternes.consultation.libelleDiagnostic")}
                          value={diagLibelle}
                          onChange={(e) => setDiagLibelle(e.target.value)}
                        />
                        <input
                          className={champ}
                          placeholder={t("medecinsExternes.consultation.codeCim")}
                          value={diagCode}
                        ...
                        <label className="flex items-center gap-2 text-xs">
                          ...
                          />
                          {t("medecinsExternes.consultation.principal")}
                        </label>
                        <button
                          type="button"
                        ...
                        >
                          ...
                          {t("medecinsExternes.consultation.ajouterDiagnostic")}
                        </button>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="rounded-xl border border-gris-bordure bg-white p-4 shadow-sm">
                    <h4 className="text-sm font-semibold">
                      {t("medecinsExternes.consultation.actes")}
                    </h4>
                    <ul className="mt-3 space-y-2">
                    ...
                    {!cloturee && consultation && (
                      <div className="mt-3 space-y-2">
                        <input
                          className={champ}
                          placeholder={t("medecinsExternes.consultation.typeActe")}
                          value={acteType}
                          onChange={(e) => setActeType(e.target.value)}
                        />
                        <input
                          className={champ}
                          placeholder={t("medecinsExternes.consultation.libelleActe")}
                          value={acteLibelle}
                        ...
                        >
                          <Plus className="h-3.5 w-3.5" />
                          {t("medecinsExternes.consultation.ajouterActe")}
                        </button>
                      </div>
                    )}
                  </div>
                  ...
                  {!cloturee && (
                    <>
                      ...
                      >
                        ...
                        {t("medecinsExternes.consultation.enregistrer")}
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        ...
                        {t("medecinsExternes.consultation.cloturer")}
                      </button>
                    </>
                  )}
                  {consultation && (
                    ...
                    >
                      ...
                      {t("medecinsExternes.consultation.imprimerCr")}
                    </button>
                  )}
                  {cloturee && (
                    <Link
                    ...
                    >
                      {t("medecinsExternes.consultation.lienOrientation")}
                    </Link>
                  )}
                </div>
              </>
