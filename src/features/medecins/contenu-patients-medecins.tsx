...
interface PropsContenuPatientsMedecins {
            ...
            <table className="tableau-sigh">
              <thead className="border-b border-gris-bordure bg-gris-tres-clair/60 text-xs uppercase tracking-wide text-texte-secondaire">
                <tr>
                  ...
                  <th className="px-2 py-1.5">{t("medecins.patients.colonnes.ordre")}</th>
                  ...
                  <th className="px-2 py-1.5">{t("medecins.patients.colonnes.provenance")}</th>
...
export function ContenuPatientsMedecins({ utilisateur }: PropsContenuPatientsMedecins) {
  const { t } = useTranslation();
  ...
  const charger = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setErreur(data.erreur ?? t("medecins.patients.erreur"));
        setPatients([]);
        return;
      }
      setPatients(data.patients ?? []);
    } catch {
      setErreur(t("medecins.patients.erreur"));
    } finally {
      setChargement(false);
    }
  }, [t]);
  ...
  return (
    <MiseEnPageMedecins
      ...
      titre={t("medecins.patients.titre")}
      sousTitre={t("medecins.patients.sousTitre")}
