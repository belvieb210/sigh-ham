...
interface PropsMenuActionsTransfertMedecins {
  patient: PatientFileMedecins;
  onRafraichir?: () => void;
}

export function MenuActionsTransfertMedecins({
  patient,
  onRafraichir,
}: PropsMenuActionsTransfertMedecins) {
  ...
  const executerAction = async (action: "confirmer" | "rejeter" | "restaurer") => {
    ...
    try {
      const res = await fetch(
        `/api/medecins/transferts/${encodeURIComponent(patient.transfertSortantId)}/actions`,
        {
          method: "POST",
      ...
      if (!res.ok) throw new Error(data.message ?? t("medecins.actions.actionImpossible"));
      ...
      setErreur(
        error instanceof Error ? error.message : t("medecins.actions.erreurInattendue")
      );
    } finally {
      setEnCours(false);
    }
  ...
  return (
    <>
      <button
        ...
        aria-label={t("medecins.actions.plusActions", { nom: patient.nomComplet })}
          ...
          >
            {peutConfirmer && (
              ...
              >
                ...
                <span>{t("medecins.actions.confirmer")}</span>
              </button>
            )}
            {peutRejeter && (
              <button
              ...
              >
                ...
                <span>{t("medecins.actions.rejeter")}</span>
              </button>
            )}
            {peutRestaurer && (
              <button
