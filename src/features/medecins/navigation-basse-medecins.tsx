...
interface PropsNavigationBasseMedecins {
  onMenu: () => void;
}

export function NavigationBasseMedecins({ onMenu }: PropsNavigationBasseMedecins) {
  const { t } = useTranslation();
  const pathname = usePathname();
  const { basse } = useNavigationMedecins();
  ...
  return (
    <nav
      ...
      aria-label={t("medecins.layout.navigationMobile")}
    >
      <ul className="flex items-end justify-around px-1 pt-1">
        {basse.map((item) => {
          const actif =
            ...
            (item.id === "ordonnances" &&
              pathname.startsWith("/sigh/medecins/ordonnances"));
        ...
        <li className="flex-1">
          ...
          >
            ...
            <span>{t("medecins.navBas.menu")}</span>
          </button>
        </li>
      </ul>
    </nav>
