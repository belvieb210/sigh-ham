...
interface PropsOrientationRapideMedecins {
  orientation?: string;
...
export function OrientationRapideMedecins({
  ...
  return (
    <div className="space-y-2">
      {ORIENTATIONS_RAPIDES_MEDECINS.map((opt) => {
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block">
                {t(`medecins.orientations.${opt.value}.label`)}
              </span>
              <span className="block text-xs font-normal text-texte-secondaire">
                {t(`medecins.orientations.${opt.value}.description`)}
              </span>
            </span>
          </button>
        );
