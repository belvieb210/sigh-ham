...
import type { UtilisateurPharmacie } from "@/lib/auth/props-utilisateur-pharmacie";
import { cn } from "@/lib/utils";

interface PropsBarreLateralePharmacie {
...
function LiensNavigation({
  ...
  const { tableauDeBord, pharmacie, communication, parametres } =
    useNavigationPharmacie();

  const estActif = (href: string) => {
    if (href === "/sigh/pharmacie") return pathname === href;
    ...
    <>
      <div className="flex items-start justify-between border-b border-gris-bordure px-4 py-4">
        <div className="min-w-0 flex-1">
          <LogoHam taille="petit" href="/sigh/pharmacie" className="max-w-full" />
        ...
        {onFermer && (
          <button
            ...
            aria-label={t("pharmacie.layout.fermerMenu")}
          >
            <X className="h-5 w-5" />
          </button>
        )}
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        {rendreSection(
          t("pharmacie.layout.sectionTableauDeBord"),
          tableauDeBord as ItemNav[]
        )}
        {rendreSection(t("pharmacie.layout.sectionPharmacie"), pharmacie as ItemNav[])}
        {rendreSection(
          t("pharmacie.layout.sectionCommunication"),
          communication as ItemNav[]
        )}
        {rendreSection(t("pharmacie.layout.sectionParametres"), parametres as ItemNav[])}
      </nav>

      <div className="space-y-2 border-t border-gris-bordure p-3">
        <Link
          href="/sigh/pharmacie/profil"
          onClick={onFermer}
        ...
        {onFermer && (
          ...
          >
            ...
            {t("pharmacie.layout.deconnecter")}
          </button>
        )}
      </div>
    </>
...
export function BarreLateralePharmacie({
  utilisateur,
  ouvert = false,
  onFermer,
  badgeFile = 0,
}: PropsBarreLateralePharmacie) {
  const { t } = useTranslation();
...
interface PropsEnTetePharmacie {
  titre: string;
  sousTitre: string;
  utilisateur: UtilisateurPharmacie;
  onMenu?: () => void;
...
export function EnTetePharmacie({
  titre,
  sousTitre,
  utilisateur,
  onMenu,
}: PropsEnTetePharmacie) {
  const { t } = useTranslation();
    ...
    <header className="sticky top-0 z-30 shrink-0 border-b border-gris-bordure bg-white">
      <div className="flex items-center gap-1.5 px-2.5 py-2 sm:gap-2 sm:px-4 sm:py-2.5 lg:hidden">
        <button
          ...
          aria-label={t("pharmacie.layout.ouvrirMenu")}
        >
        ...
        <MenuProfilEntete utilisateur={utilisateur} compact hrefProfil="/sigh/pharmacie/profil" />
      </div>
      ...
      <div className="hidden items-center justify-between gap-4 px-6 py-4 lg:flex">
        ...
        <div className="relative hidden min-w-0 flex-1 max-w-md xl:block">
          ...
          <input
            ...
            placeholder={t("pharmacie.layout.rechercher")}
          ...
          <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 rounded border border-gris-bordure bg-white px-1.5 py-0.5 text-[10px] text-texte-secondaire">
            {t("pharmacie.layout.raccourciRecherche")}
          </span>
        </div>
        ...
        <div className="flex items-center justify-end gap-3">
          <SelecteurLangue />
          <BoutonNotificationsEnTete />
          <MenuProfilEntete utilisateur={utilisateur} hrefProfil="/sigh/pharmacie/profil" />
        </div>
      </div>
    </header>
  );
