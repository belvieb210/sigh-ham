...
interface PropsMenuActionsTransfertPharmacie {
  patient: PatientFilePharmacie;
  onRafraichir?: () => void;
}

export function MenuActionsTransfertPharmacie({
  patient,
  onRafraichir,
}: PropsMenuActionsTransfertPharmacie) {
  ...
  const executerAction = async (action: "confirmer" | "rejeter" | "restaurer") => {
    ...
    try {
      const res = await fetch(
        `/api/pharmacie/transferts/${encodeURIComponent(patient.transfertSortantId)}/actions`,
        {
          method: "POST",
      ...
      if (!res.ok) throw new Error(data.message ?? t("pharmacie.actions.actionImpossible"));
      ...
      setErreur(
        error instanceof Error ? error.message : t("pharmacie.actions.erreurInattendue")
      );
    } finally {
      setEnCours(false);
    }
  ...
  return (
    <>
      <button
        ...
        aria-label={t("pharmacie.actions.plusActions", { nom: patient.nomComplet })}
          ...
          >
            {peutConfirmer && (
              ...
              >
                ...
                <span>{t("pharmacie.actions.confirmer")}</span>
              </button>
            )}
            {peutRejeter && (
              <button
              ...
              >
                ...
                <span>{t("pharmacie.actions.rejeter")}</span>
              </button>
            )}
            {peutRestaurer && (
              <button
