...
interface PropsNavigationBasseReception {
  onMenu: () => void;
}

export function NavigationBasseReception({ onMenu }: PropsNavigationBasseReception) {
  const { t } = useTranslation();
  const pathname = usePathname();
  const { basse } = useNavigationReception();
  ...
  return (
    <nav
      ...
      aria-label={t("reception.layout.navigationMobile")}
    >
      <ul className="flex items-stretch justify-around px-1 pt-1">
        {basse.map((item) => {
          const actif =
            pathname === item.href ||
            (item.id === "accueil" && pathname === "/sigh/reception") ||
            (item.id === "nouveau" && pathname.startsWith("/sigh/reception/nouveau")) ||
            (item.id === "patients" && pathname.startsWith("/sigh/reception/enregistres")) ||
            (item.id === "transferes" && pathname.startsWith("/sigh/reception/transferts"));
        ...
        <li className="flex-1">
          ...
          >
            ...
            <span>{t("reception.layout.menu")}</span>
          </button>
        </li>
      </ul>
    </nav>
