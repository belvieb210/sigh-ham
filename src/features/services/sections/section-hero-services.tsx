...
import { ArrowRight, Calendar, ChevronDown, Stethoscope } from "lucide-react";
...
export function SectionHeroServices() {
  const { hero, services } = useContenuServices();
  const imagesFond =
    hero.imagesFond.length > 0
      ? hero.imagesFond
      : [{ url: IMAGE_HERO_SERVICES, alt: "Services HAM Laboratoire" }];

  return (
    <section
      className="section-hero-services relative overflow-hidden"
      aria-labelledby="titre-services-hero"
    >
      <CarrouselFondSectionHero
        images={imagesFond}
        gradientClassName="bg-gradient-to-r from-[#0f172a]/95 via-[#1a4d7c]/90 to-[#2d2a6e]/85"
        altFallback="Services HAM Laboratoire"
      />
        ...
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          >
            ...
            <p className="mt-6 text-sm text-white/60">
              {services.length} prestations · Laboratoire certifié ISO 9001:2015
            </p>
          </motion.div>
