...
async function chargerTransfertInfirmiers(transfertId: string) {
  ...
  if (transfert.salleOrigine.code !== "INFIRMIERS") {
    throw new Error("Ce transfert ne peut être géré que depuis la salle infirmiers.");
  }

  return transfert;
}

export async function confirmerTransfertInfirmiers(
  agentId: string,
  transfertId: string
) {
  const transfert = await chargerTransfertInfirmiers(transfertId);
  ...
  const resultat = await prisma.$transaction(async (tx) => {
    const fileinfirmiers = await tx.fileAttente.findFirst({
      where: {
        passageId: transfert.passageId!,
        serviLe: null,
        salle: { code: "INFIRMIERS" },
      },
    });

    if (fileinfirmiers) {
      await tx.fileAttente.update({
        where: { id: fileinfirmiers.id },
    ...
    const transfertEntrant = await tx.transfert.findFirst({
      where: {
        passageId: transfert.passageId!,
        salleDestination: { code: "INFIRMIERS" },
...
export async function rejeterTransfertInfirmiers(
  agentId: string,
  transfertId: string,
  motifRejet?: string
) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await tx.transfertRecuperation.upsert({
      ...
      create: {
        ...
        patientId: transfert.dossier.patientId,
        examensIds: [],
        motifRejet: motifRejet?.trim() || "Transfert rejeté chez les infirmiers",
        rejeteParId: agentId,
      },
      update: {
...
export async function restaurerTransfertInfirmiers(
  agentId: string,
  transfertId: string
) {
  const transfert = await chargerTransfertInfirmiers(transfertId);
