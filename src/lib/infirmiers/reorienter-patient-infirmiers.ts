...
import { CODES_ORIENTATION_INFIRMIERS } from "@/constants/infirmiers";
import { prisma } from "@/lib/prisma";
import { synchroniserTransfertsEnAttente } from "@/lib/transferts/multi-destinations";

const ORIENTATIONS_AUTORISEES = new Set<string>(CODES_ORIENTATION_INFIRMIERS);
...
function normaliserDestinations(orientations: string[]): CodeSalle[] {
  ...
  if (codes.includes("INFIRMIERS")) {
    throw new Error(
      "Le patient est déjà chez les infirmiers. Choisissez une autre destination."
    );
  }
  return codes;
}

/** Synchronise orientations rapides infirmiers (1 ou plusieurs salles). */
export async function reorienterPatientDepuisInfirmiers(
  agentId: string,
  dossierId: string,
  ...
  if (!passage?.fileAttente) {
    throw new Error("Patient introuvable dans la file d'attente infirmiers.");
  }
  ...
  const resultat = await synchroniserTransfertsEnAttente({
    ...
    salleOrigineId: salleOrigine.id,
    destinations,
    motifPrefixe: "Orientation rapide infirmiers",
  });
