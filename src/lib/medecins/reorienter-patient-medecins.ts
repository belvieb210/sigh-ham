...
function normaliserDestinations(orientations: string[]): CodeSalle[] {
  ...
  if (codes.includes("MEDECINS")) {
    throw new Error(
      "Le patient est déjà chez les médecins. Choisissez une autre destination."
    );
  }
  return codes;
}
...
/**
 * Synchronise orientations rapides médecins (1 ou plusieurs salles).
 */
export async function reorienterPatientDepuisMedecins(
  agentId: string,
  dossierId: string,
  ...
  if (!passage?.fileAttente) {
    throw new Error("Patient introuvable dans la file d'attente médecins.");
  }
  ...
  const resultat = await synchroniserTransfertsEnAttente({
    ...
    salleOrigineId: salleOrigine.id,
    destinations,
    motifPrefixe: "Orientation rapide médecins",
  });
