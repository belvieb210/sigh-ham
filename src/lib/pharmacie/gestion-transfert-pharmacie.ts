...
async function chargerTransfertPharmacie(transfertId: string) {
  ...
  if (transfert.salleOrigine.code !== "PHARMACIE") {
    throw new Error("Ce transfert ne peut être géré que depuis la salle pharmacie.");
  }

  return transfert;
}

export async function confirmerTransfertPharmacie(
  agentId: string,
  transfertId: string
) {
  const transfert = await chargerTransfertPharmacie(transfertId);
...
export async function rejeterTransfertPharmacie(
  agentId: string,
  transfertId: string,
  motifRejet?: string
) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await tx.transfertRecuperation.upsert({
      ...
      create: {
        ...
        patientId: transfert.dossier.patientId,
        examensIds: [],
        motifRejet: motifRejet?.trim() || "Transfert rejeté chez les pharmacie",
        rejeteParId: agentId,
      },
      update: {
...
export async function restaurerTransfertPharmacie(
  agentId: string,
  transfertId: string
) {
  const transfert = await chargerTransfertPharmacie(transfertId);
